...
## 2026-06-04（续）：阶段 8 — 轨迹线绘制与运动轨迹可视化

### 1. 功能目标

在 ByteTrack 多目标跟踪基础上，为目标绘制最近 N 帧运动轨迹，增强视频展示效果，帮助观察风险目标运动过程。

### 2. 实现方案

- **轨迹点**：检测框底边中心点 `((x1+x2)/2, y2)`，作为地面近似点
- **历史长度**：`TRAJECTORY_HISTORY = 30`，使用 `deque(maxlen=30)` 自动截断
- **数据结构**：`defaultdict(lambda: deque(maxlen=TRAJECTORY_HISTORY))`，key 为 track_id
- **绘制**：`cv2.polylines()` 绘制折线，仅对当前帧可见目标绘制
- **颜色区分**：
  - 普通目标：浅青绿色
  - 车辆风险目标：偏红色
  - VRU 风险目标：偏品红色
- **绘制顺序**：轨迹线 → 风险区域 → 检测框 → 风险标记（框覆盖在轨迹之上）

### 3. 运行统计

两个视频的统计与阶段 6 完全一致，轨迹线为纯视觉增强，不改变统计口径。

| 视频 | 帧数 | 稳定车辆风险 | 稳定 VRU 风险 | 说明 |
|------|------|-------------|--------------|------|
| road_drive_01 | 722 | 498 (69.0%) | 0 | 车辆轨迹线持续 502 帧 |
| road_city_vru_01 | 1800 | 0 | 1687 (93.7%) | 180 个稳定 VRU ID 显示轨迹 |

### 4. 输出视频

| 视频 | 输出路径 |
|------|----------|
| road_drive_01 | `outputs/videos/road_drive_01_tracked_with_traj.mp4` |
| road_city_vru_01 | `outputs/videos/road_city_vru_01_tracked_with_traj.mp4` |

### 5. 修改文件

| 文件 | 变更 |
|------|------|
| `scripts/track_video.py` | 新增轨迹缓存、记录、绘制逻辑；新增 `get_anchor_point`、`draw_trajectories`、`record_track_positions` |
| `README.md` | 功能特点新增轨迹线；项目结构更新；后续计划标记完成；进度清单追加 |
| `PROJECT_LOG.md` | 追加阶段 8 记录 |

### 6. Git 提交

```
feat: add trajectory visualization for tracked objects
```
...
## 2026-06-04（续）：阶段 9B — 最终项目总结 + 简历包装 + 面试讲解稿

### 1. 新增文档

`PROJECT_SUMMARY.md` — 项目最终总结文档，包含：

- 项目名称（中英文）
- 项目简介
- 核心功能（9 项）
- 技术路线（6 步流程 + 每步解释）
- 风险判断规则（车辆/VRU/稳定/轨迹线）
- 测试结果摘要（两段视频完整数据）
- 项目亮点（7 条）
- 当前局限（8 条）
- 后续改进方向（8 项）
- 项目结构
- **Resume Version**（简历精简版 + 面试详细版）
- **Interview Script**（2–3 分钟面试讲解稿）
- **Interview Q&A**（12 个常见面试问答）
- 开发时间线（15 个阶段完整追溯）

### 2. README 更新

新增 `## Project Summary` 小节，指向 `PROJECT_SUMMARY.md`。

### 3. 项目收尾状态

- 15 个开发阶段全部完成
- 17 个 Git commits，每次均有明确提交信息
- GitHub 仓库包含：完整代码、效果展示图、项目文档、面试资料
- 工作区干净，已同步远程
...
| 文件 | 变更 |
|------|------|
| `PROJECT_SUMMARY.md` | 新增：最终项目总结 + 简历 + 面试稿 + Q&A |
| `README.md` | 新增 Project Summary 小节 |
| `PROJECT_LOG.md` | 追加阶段 9B 记录 |

### 5. Git 提交

```
docs: add final project summary and interview notes
```

---

*项目阶段性开发记录到此结束。后续功能迭代将继续在此基础上展开。*
